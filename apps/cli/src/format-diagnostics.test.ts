...
describe('formatDiagnostic', () => {
	describe('W004: Circular dependency', () => {
		...
		it('formats self-referential cycle', () => {
			...
			expect(result).toBe(
				'broken.siren:1:10: W004: Circular dependency detected: self -> self',
			);
		});
	});
	...
	describe('E001: Parse error', () => {
		it('formats parse error', () => {
			...
			expect(result).toBe(
				'broken.siren:42:15: E001: Invalid syntax: unexpected token',
			);
		});
	});
	...
	describe('Edge cases', () => {
		it('handles diagnostic with zero column', () => {
			...
			expect(result).toBe(
				'test.siren:1:0: W004: Circular dependency detected: a -> b -> a',
			);
		});
