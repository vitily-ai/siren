 ...
 *
 * Modules are appended via `.pipe(module)`. Each module's input type must be
 * a subset of the cumulative envelope. The runner:
 *
 *  1. Deep-freezes the seed envelope.
 *  2. For each module: invokes `run(envelope)`, deep-freezes the additions, and
 ...
 *     `graph` with a completion-resolved EntryGraph snapshot).
 *  3. Returns the terminal envelope (frozen).
 *
 * Type-level guarantees:
 *  - `.pipe(m)` requires `m`'s declared input type to be a structural subset
...
export class Pipeline<TSeed extends Envelope, TEnv extends Envelope> {
  ...
  pipe<TIn extends Envelope, TAdd extends Envelope>(
    ...
    return new Pipeline<TSeed, Omit<TEnv, keyof TAdd> & TAdd>([
      ...this.modules,
      module as Module<Envelope, Envelope>,
    ]);
  }

  run(seed: TSeed): TEnv {
    let envelope: Envelope = deepFreeze({ ...seed });
    for (const module of this.modules) {
      const additions = deepFreeze(module.run(envelope));
      envelope = deepFreeze({ ...envelope, ...additions });
    }
