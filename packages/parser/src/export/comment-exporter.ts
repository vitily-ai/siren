import type { AttributeValue, Resource } from '@siren/core';
import type { DecodedAttribute, DecodedResource } from '../decoder/decoded-types.js';
...
export function exportWithComments(
  ...
  if (!sourceIndex) {
    return exportToSiren(resources);
  }
  ...
  const resSorted = resources.slice();
  // If origin exists, prefer stable byte order (matches the file for per-file IR)
  resSorted.sort((a, b) => {
    const ao = (a as DecodedResource)._origin?.startByte ?? Number.POSITIVE_INFINITY;
    const bo = (b as DecodedResource)._origin?.startByte ?? Number.POSITIVE_INFINITY;
    return ao - bo;
  });
  ...
  function flushTopLevelCommentsUntil(endByteExclusive: number): void {
    ...
    while (commentIdx < allComments.length) {
      ...
      const owningRes = resSorted.find((r) => {
        const o = (r as DecodedResource)._origin;
        return o != null && c.startByte >= o.startByte && c.endByte <= o.endByte;
      });
      if (owningRes) {
        commentIdx++;
        continue;
  ...
  for (const res of resSorted) {
    const resStart = (res as DecodedResource)._origin?.startByte ?? Number.POSITIVE_INFINITY;
    const resEnd = (res as DecodedResource)._origin?.endByte ?? Number.NEGATIVE_INFINITY;
    ...
    const headerRow = (res as DecodedResource)._origin?.startRow;
    ...
    for (const attr of res.attributes) {
      const order = (attr as DecodedAttribute)._origin?.startByte ?? Number.POSITIVE_INFINITY;
      const entryIndex = bodyEntries.length;
      const endRow = (attr as DecodedAttribute)._origin?.endRow;

      bodyEntries.push({
        order,
        seq: seq++,
        text: formatAttributeLine(attr.key, attr.value, (attr as DecodedAttribute)._raw),
      });
      ...
      if (endRow !== undefined) {
        attributeByEndRow.set(endRow, {
          ...
          key: attr.key,
          value: attr.value,
          raw: (attr as DecodedAttribute)._raw,
        });
      }
    }
      ...
      if (c.startByte >= resStart && c.endByte <= resEnd) {
        ...
        if (headerRow !== undefined && c.startRow === headerRow) {
          ...
          if (attrOnSameLine) {
            ...
            const entry = bodyEntries[entryIndex]!;
            // Re-format the attribute line with the trailing comment
            entry.text = formatAttributeLine(
              attrOnSameLine.key,
    ...
    segments.push({
      kind: 'resource',
      lines: block.split('\n'),
      startRow: (res as DecodedResource)._origin?.startRow ?? 0,
      endRow: (res as DecodedResource)._origin?.endRow ?? 0,
    });
  }
