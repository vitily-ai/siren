...
import type { DecodedResource } from '../decoder/decoded-types.js';
import type { CommentToken, Origin } from '../parser/cst.js';
import { SourceIndex } from '../parser/source-index.js';
import { exportWithComments } from './comment-exporter.js';
...
describe('exportWithComments', () => {
  describe('fallback behavior', () => {
    it('should call exportToSiren when no SourceIndex provided', () => {
      const resources: DecodedResource[] = [
        {
          type: 'task',
          id: 'task1',
          complete: false,
      ...
      const resultWithoutIndex = exportWithComments(resources);
      // Export with exportToSiren directly
      const resultWithExportToSiren = exportToSiren(resources);
      ...
      const resources: DecodedResource[] = [
        {
          ...
          complete: false,
          attributes: [],
          _origin: origin(19, 32, 1, 1),
        },
      ];
      const result = exportWithComments(resources, index);
  ...
  describe('detached comment blocks', () => {
    it('should preserve detached block with blank line separation', () => {
      ...
      const result = exportWithComments(resources, indexForDetached);
  ...
  describe('edge cases', () => {
    it('should handle empty IR with comments gracefully', () => {
      ...
      const result = exportWithComments([], index);
    ...
    it('should handle IR with no comments (semantic-only export)', () => {
      ...
      const result = exportWithComments(resources);

      // Should match semantic-only export
      expect(result).toBe(exportToSiren(resources));
    });
